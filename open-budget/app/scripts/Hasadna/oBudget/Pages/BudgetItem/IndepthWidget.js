...
export default class IndepthWidget extends Backbone.View {
  ...
  render__chart_bg() {
    ...
    if (this.tipBG == null) {
      ...
      this.tipBGleft = this.chart.append('rect').style('fill', '#ccc').style('opacity', 0.05).attr('y', 0).attr('height', TOP_PART_SIZE).attr('x', 0);
      this.tipBGright = this.chart.append('rect').style('fill', '#ccc').style('opacity', 0.05).attr('y', 0).attr('height', TOP_PART_SIZE).attr('x', 0).attr('width', 10000);
    }
    ...
    this.chart.selectAll('.graduationLine').data(allLabelIndexes).enter().append('line').attr('class', d => `graduationLine ${ d.major ? 'major' : 'minor' }`);
    this.chart.selectAll('.graduationLine').data(allLabelIndexes).attr('x1', d => this.timeScale(this.minTime)).attr('x2', d => this.timeScale(this.maxTime)).attr('y1', d => this.valueScale(this.minValue + d.index * this.tickValue)).attr('y2', d => this.valueScale(this.minValue + d.index * this.tickValue));
    const graduationLabels = this.chart.selectAll('.graduationLabel').data(_.filter(allLabelIndexes, x => x.major));
    return graduationLabels.enter().append('text').attr('class', 'graduationLabel').attr('x', d => this.timeScale(this.minTime)).attr('y', d => this.valueScale(this.minValue + d.index * this.tickValue)).attr('dx', 10).attr('dy', -1).style('font-size', 8).style('text-anchor', 'end').text(d => this.formatNumber(this.minValue + d.index * this.tickValue));
  }
  render__guideline() {
    ...
    ]).enter().append('line').attr('class', 'guideline').attr('y1', 0).attr('y2', this.$el.height()).style('stroke', d => d.s).style('stroke-width', d => d.w).style('pointer-events', 'none');
    d3.select('body').selectAll('#indepth-guideline-date').data([1]).enter().append('div').attr('id', 'indepth-guideline-date');
  ...
  render__year_starts() {
    ...
    const newGraphParts = this.chart.selectAll('.graphPartYearStart').data(yearstartModels).enter().append('g').attr('class', 'graphPartYearStart');
    newGraphParts.append('line').attr('class', 'yearstartLine').datum(d => d);
    newGraphParts.append('text').attr('class', 'yearstartLabel').style('font-size', 12).attr('dx', 3).text(d => d.get('date').getFullYear()).style('text-anchor', 'end').datum(d => d);
    this.chart.selectAll('.yearstartLine').data(yearstartModels).attr('class', 'yearstartLine').attr('x1', d => this.timeScale(d.get('timestamp'))).attr('x2', d => this.timeScale(d.get('timestamp'))).attr('y1', d => this.valueScale(d.get('value'))).attr('y2', d => this.valueScale(this.minValue) + YEAR_LINE_HANG_LENGTH);
    return this.chart.selectAll('.yearstartLabel').data(yearstartModels).attr('x', d => this.timeScale(d.get('timestamp'))).attr('y', d => this.valueScale(this.minValue) + YEAR_LINE_HANG_LENGTH);
  }
  render__approved_budgets() {
    const approvedModels = _.filter(this.model.models, x => x.get('kind') === 'approved');
    const newGraphParts = this.chart.selectAll('.graphPartApproved').data(approvedModels).enter().append('g').attr('class', 'graphPartApproved');
    newGraphParts.append('line').attr('class', 'approvedLine').datum(d => d);
    newGraphParts.append('line').attr('class', 'approvedBar').datum(d => d);
    const newTips = d3.select('#approvedTooltips').selectAll('.approvedTip').data(approvedModels).enter().append('div').attr('class', 'approvedTip participantTooltip').html(d => tplWidgetParticipantTooltip({
      participants: d.get('participants'),
      _
    }));
    d3.select('#approvedTooltips').selectAll('.approvedTip').data(approvedModels).style('left', d => this.timeScale(d.get('timestamp')) + 'px').style('top', '0px');
    return this.chart.selectAll('.approvedBar').data(approvedModels).attr('x1', d => this.timeScale(d.get('timestamp'))).attr('x2', d => this.timeScale(d.get('timestamp') + d.get('width'))).attr('y1', d => this.valueScale(d.get('value'))).attr('y2', d => this.valueScale(d.get('value')));
  }
  render__revised_budgets() {
    let dby;
    const revisedModels = _.filter(this.model.models, x => x.get('kind') === 'revised' && !x.get('disabled'));
    const newGraphParts = this.chart.selectAll('.graphPartRevised').data(revisedModels).enter().append('g').attr('class', 'graphPartRevised');
    newGraphParts.append('line').attr('class', 'revisedBar').datum(d => d);
    return this.chart.selectAll('.revisedBar').data(revisedModels).attr('class', d => (dby = d.get('diff_baseline'), dby < 0 ? 'revisedBar reduce' : dby > 0 ? 'revisedBar increase' : 'revisedBAr')).attr('x1', d => this.timeScale(d.get('timestamp') - d.get('width'))).attr('x2', d => this.timeScale(d.get('timestamp'))).attr('y1', d => this.valueScale(d.get('original_baseline'))).attr('y2', d => this.valueScale(d.get('value')));
  }
  render__change_items() {
    ...
    this.chart.selectAll('.changeBar-last').data(lastChanges).enter().append('rect').attr('class', 'changeBar-last').datum(d => d);
    this.chart.selectAll('.changeBar-last-line').data(lastChanges).enter().append('line').attr('class', 'changeBar-last-line').datum(d => d);
    const newGraphParts = this.chart.selectAll('.graphPartChanged').data(changeModels).enter().append('g').attr('class', 'graphPartChanged');
    ...
    this.chart.selectAll('.changeBar-last').data(lastChanges).attr('class', d => (cls = changeClass(d.get('original_baseline'), d.get('value')), subkind = d.get('subkind'), `changeBar-last ${ cls } ${ subkind }`)).attr('x', d => this.timeScale(this.roundToYearStart(d.get('timestamp')))).attr('width', d => this.timeScale(d.get('timestamp') + d.get('width')) - this.timeScale(this.roundToYearStart(d.get('timestamp')))).attr('y', d => this.valueScale(d.get('value'))).attr('height', d => this.valueScale(this.minValue) - this.valueScale(d.get('value')));
    this.chart.selectAll('.changeBar-last-line').data(lastChanges).attr('class', d => (cls = changeClass(d.get('original_baseline'), d.get('value')), subkind = d.get('subkind'), `changeBar-last-line ${ cls } ${ subkind }`)).attr('x1', d => this.timeScale(this.roundToYearStart(d.get('timestamp')))).attr('x2', d => this.timeScale(d.get('timestamp'))).attr('y1', d => this.valueScale(d.get('value'))).attr('y2', d => this.valueScale(d.get('value')));
  ...
  render() {
    ...
    if (this.show_changes) {
      ...
      this.render__revised_budgets();
      this.render__change_items();
      this.render__used_budgets();
    } else {
      this.render__yearly_lines();
    }
    this.render__tooltip_hooks();
    this.render__timeline_terms();
    this.render__timeline_titles();
    return this.render__guideline();
  }
  formatNumber(n) {
    const rx = /(\d+)(\d{3})/;
